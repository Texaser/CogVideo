...
class CogVideoXControlnet(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def __init__(
        ...
        # Add zero convolutions for each control stage
        self.zero_conv_first = nn.Conv2d(input_channels[1], input_channels[1], kernel_size=1)
        self.zero_conv_second = nn.Conv2d(input_channels[2], input_channels[2], kernel_size=1)
        
        # Initialize zero convolutions with zeros
        nn.init.zeros_(self.zero_conv_first.weight)
        nn.init.zeros_(self.zero_conv_first.bias)
        nn.init.zeros_(self.zero_conv_second.weight)
        nn.init.zeros_(self.zero_conv_second.bias)
        ...
        # Add zero convolutions for transformer outputs
        if out_proj_dim is not None:
            self.out_projectors = nn.ModuleList(
                [nn.Linear(inner_dim, out_proj_dim) for _ in range(num_layers)]
            )
            # Add zero projections after each transformer block
            self.zero_projs = nn.ModuleList(
                [nn.Linear(out_proj_dim, out_proj_dim) for _ in range(num_layers)]
            )
            # Initialize zero projections with zeros
    ...
    def forward(
        ...
        # Apply first control encoder and zero conv
        controlnet_states = self.controlnet_encode_first(controlnet_states)
        controlnet_states = self.zero_conv_first(controlnet_states)
        ...
        # Apply second control encoder and zero conv
        controlnet_states = self.controlnet_encode_second(controlnet_states)
        controlnet_states = self.zero_conv_second(controlnet_states)
        controlnet_states = self.compress_time(controlnet_states, num_frames=num_frames) 
        ...
        # 3. Transformer blocks with zero projections
        for i, block in enumerate(self.transformer_blocks):
            ...
            if self.out_projectors is not None:
                # Apply projection and zero projection
