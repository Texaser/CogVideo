...
def save_video_as_grid_and_mp4(
    ...
    """
    Save reconstructed latents `video_batch` to `save_path` as MP4s.
...
def sampling_main(args, model_cls):
    ...
    with torch.no_grad():
        for text, cnt in tqdm(data_iter):
            ...
            # [text, initial image path] pairs
            if args.image2video:
                text, image_path = text.split("@@")
                assert os.path.exists(image_path), image_path
                image = Image.open(image_path).convert("RGB")
                image = transform(image).unsqueeze(0).to("cuda")
                image = resize_for_rectangle_crop(
                    image, image_size, reshape_mode="center"
                ).unsqueeze(0)
                image = image * 2.0 - 1.0
                ...
                image = torch.concat(
                    [image, torch.zeros(pad_shape).to(image.device).to(image.dtype)],
                    dim=1,
                )
            else:
                image = None
            ...
            for index in range(args.batch_size):
                ...
                # output path for resulting sample
                ...
                # save latent as mp4 to `save_path`
                if mpu.get_model_parallel_rank() == 0:
                    save_video_as_grid_and_mp4(
                        samples, save_path, fps=args.sampling_fps
                    )
