...
def resize_video_tensor_bilinear(input_tensor, video_shape):
    """
    Resizes the input tensor from shape (T, C, H, W) to (T, C, H1, W1).

    Args:
        input_tensor (torch.Tensor): The input tensor of shape (T, C, H, W).

    Returns:
        torch.Tensor: The resized tensor of shape (T, C, H1, W1).
    """
    
    H, W = video_shape[0], video_shape[1]
    output_tensor = F.interpolate(
        input_tensor, size=(H, W), mode='bilinear', align_corners=False, antialias=True
    )
